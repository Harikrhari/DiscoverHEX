...
// ── Color swatch mock data keyed by tag ─────────────────────────────────────
...
function getSwatches(tags, category) {
  if (!tags) return COLOR_SWATCHES.default;
...
export default function ProductDetail() {
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 text-center">
        ...
        <p className="text-gray-500 mb-8 max-w-sm">
          We couldn't find the product you're looking for. It may have been removed or the link is
          incorrect.
        </p>
        <button
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-2 bg-hex-primary text-white font-bold px-6 py-3 rounded-xl hover:bg-orange-600 transition-colors mr-3"
        >
          <ChevronLeft size={18} />
          Go Back
        </button>
  ...
  const charityDonation = ((product.price * quantity * (product.charityPercent || 5)) / 100).toFixed(2);

  // Pad images array so there's always at least 4 thumbnails (repeat the same image)
  const images =
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-6 pb-2">
        <nav className="flex items-center gap-2 text-sm text-gray-400">
          ...
          <span>/</span>
          <span className="text-hex-secondary font-medium truncate max-w-[200px]">{product.name}</span>
        </nav>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="relative rounded-2xl overflow-hidden bg-white shadow-md aspect-square">
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                {product.badge && (
                  <span className="bg-hex-accent text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg">
                    {product.badge}
                  </span>
                )}
                {discount && discount > 0 && (
              ...
              {product.sponsored && (
                <div className="absolute bottom-4 left-4 right-4">
                  <span className="inline-flex items-center gap-1.5 bg-hex-secondary/80 text-white text-xs font-medium px-3 py-1.5 rounded-full backdrop-blur-sm">
                    <Award size={12} className="text-hex-gold" />
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="bg-hex-green/10 border border-hex-green/30 rounded-xl p-4 flex items-start gap-3">
              <Heart size={20} className="text-hex-green fill-hex-green flex-shrink-0 mt-0.5" />
              <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {swatches.map((swatch, idx) => (
                  <button
                    ...
                    style={{ backgroundColor: swatch.hex, outline: '2px solid #e5e7eb' }}
            ...
            <div className="flex items-center gap-2">
              {product.inStock ? (
                <>
                  ...
                  <span className="text-xs font-semibold text-hex-green">In Stock — Ships within 1-2 business days</span>
                </>
              ) : (
                <span className="text-xs font-semibold text-red-500">Currently out of stock</span>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-4 pt-2 border-t border-gray-100">
              ...
              <div className="flex items-center gap-1.5 text-xs text-gray-500">
                <Heart size={14} className="text-hex-accent" />
                30-Day Returns
              </div>
              <div className="flex items-center gap-1.5 text-xs text-gray-500">
